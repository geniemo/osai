...
ONNX 그래프 traversal 기반 자체 counter로 측정 (Conv/Gemm/MatMul만 카운트, MAC × 2 = FLOP convention). 채점 사이트 측정값과 일치 (162.46 GFLOPs, 미세 차이는 op 카운팅 방식).
...
| **TTA** (5-scale [0.5, 0.75, 1.0, 1.25, 1.5] + hflip) | **0.8103** |

TTA는 inference에 사용. Multi-scale 평균 → 작은 객체 (low scale 우세) 와 큰 객체 (high scale 우세) 둘 다 커버. Raw → TTA에서 +3.3% 향상.
...
| 평가 | mIoU |
|---|---|
| TTA on test set (special images) | **0.785** |
...
- Colab Stage 2 best raw: **0.7542**, TTA val: ~0.79, test grading: **0.782**
- 관찰:
...
| Stage 2 best raw mIoU | 0.7542 | 0.7776 (+2.34) |
| TTA val mIoU | ~0.79 | **0.8103** |
| Test set grading mIoU | 0.782 | **0.785** |
...
TTA per-class IoU 기준 가장 낮은 5개 class:

| Class | IoU | 원인 추정 |
|---|---|---|
| chair | 0.418 | 얇은 다리 + 다양한 형태 (의자/소파/스툴), 사람과의 occlusion 빈번 |
| bicycle | 0.498 | 얇은 프레임/바퀴 (sparse mask), 사람과 자주 겹쳐 mask 분리 어려움 |
...
- **GPU**: NVIDIA L4 — Overview의 `gpu_name` 필드 확인 가능
- **Total params**: 45,076,170 (45.08M) — Overview의 `params/total`
- **Tags**: colab, voc+coco, final, copy-paste, boundary

### 7.2 결과 요약 (WandB Run summary)

| 항목 | Stage 1 | Stage 2 |
|---|---|---|
| Step | 155,000 (best) | 8,000 |
| val/best_mIoU | 0.7549 | 0.7776 |
| GPU | NVIDIA L4 | NVIDIA L4 |

### 7.3 링크 (마감 후 교수님이 직접 확인 가능)

- WandB project: <https://wandb.ai/g1nie-sungkyunkwan-university/osai-p1-colab>
- Stage 1 run name: `colab-v2.final-s1_<MMDD>_resnet50_stage1`
- Stage 2 run name: `colab-v2.final-s2_<MMDD>_resnet50_stage2`
...
을 적용해 v1 baseline 대비 채점 사이트 mIoU **0.782 → 0.785** (+0.003), val TTA mIoU **0.79 → 0.8103** (+0.02) 향상을 달성했다. FLOPs는 162.24 G로 표준 DeepLabV3+ R50 OS=16 사이즈 유지.
